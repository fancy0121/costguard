...
async function makeRuntime(codexHome: string, saveTokenHome: string) {
  return startRuntime({
...
describe("Config lifecycle real entry points", () => {
  test("install → status → sync → restore → uninstall full cycle", async () => {
    ...
    const saveTokenHome = join(tmp, "st");
    // Preset user config
  ...
  test("user edits after install survive restore/uninstall", async () => {
    ...
    try {
      ...
      // User adds a non-SaveToken file after install
      await (await import("node:fs/promises")).mkdir(saveTokenHome, { recursive: true });
      await writeFile(join(saveTokenHome, "user-notes.txt"), "user data", "utf8");
      // Restore
      await api(runtime, "/api/restore", "POST");
      // User file survives
      const content = await readFile(join(saveTokenHome, "user-notes.txt"), "utf8");
      expect(content).toBe("user data");
      // Uninstall
      await api(runtime, "/api/uninstall", "POST");
      const after = await readFile(join(saveTokenHome, "user-notes.txt"), "utf8");
  ...
  test("doctor detects journal residue", async () => {
    ...
    try {
      ...
      await (await import("node:fs/promises")).mkdir(saveTokenHome, { recursive: true });
      await writeFile(join(saveTokenHome, ".savetoken-owned-batch.json"), '{"version":1,"writes":[]}', "utf8");
      const doc = await api(runtime, "/api/doctor");
