...
test("runtime WebSocket bridge requires authorization, invokes only an allowlisted local facade, and cancels on close", async () => {
  const root = await mkdtemp(join(tmpdir(), "savetoken-ws-"));
  const calls: unknown[] = [];
  let aborted = false;
  ...
  const runtime = await startRuntime({
    env: { CODEX_HOME: join(root, "codex"), SAVETOKEN_HOME: join(root, "state") }, providers: {}, providerTier: "execution", managementToken: token, sidecarFacade: facade,
  });
  try {
