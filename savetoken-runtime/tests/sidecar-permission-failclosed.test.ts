...
test("sidecar facade rejects SaveToken control fields before an allowlisted adapter can observe them", async () => {
  ...
  await expect(connection.session.send({ query: "bounded fixture", savetokenInternal: true })).resolves.toEqual({
...
test("runtime WebSocket rejects sidecar control fields and never invokes the configured facade", async () => {
  const root = await mkdtemp(join(tmpdir(), "savetoken-sidecar-control-"));
  ...
  try {
    const result = await new Promise<unknown>((resolve, reject) => {
      const socket = new WebSocket(`${runtime.baseUrl.replace(/^http/, "ws")}/v1/sidecars/web-search`, ["savetoken-sidecar", token]);
      socket.addEventListener("open", () => socket.send(JSON.stringify({ query: "bounded fixture", savetokenControl: "local" })), { once: true });
