...
async function runtimeFor(protocol: "responses" | "chat" | "anthropic", adapter: ProviderAdapter) {
  return startRuntime({
    env: { CODEX_HOME: await mkdtemp(join(tmpdir(), "savetoken-protocol-codex-")), SAVETOKEN_HOME: await mkdtemp(join(tmpdir(), "savetoken-protocol-state-")) },
...
for (const protocol of ["responses", "chat", "anthropic"] as const) {
  test(`${protocol} stream E2E preserves streaming content type and terminal marker`, async () => {
    ...
    try {
      ...
      const response = await fetch(runtime.baseUrl + endpoint.path, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ ...endpoint.body, savetokenTask: "do not leak" }) });
      expect(response.status).toBe(200);
      expect(response.headers.get("content-type")).toContain("text/event-stream");
      expect(JSON.parse(response.headers.get("x-savetoken-route-admission") ?? "{}") as { decidingTier?: string }).toEqual(expect.objectContaining({ decidingTier: "execution" }));
      ...
      expect(stream).not.toContain("savetokenTask");
