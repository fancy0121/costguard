...
test("Chat Completions E2E preserves a native tool-call response instead of serializing it into text", async () => {
  ...
  const runtime = await startRuntime({
    env: { CODEX_HOME: await mkdtemp(join(tmpdir(), "savetoken-chat-codex-")), SAVETOKEN_HOME: await mkdtemp(join(tmpdir(), "savetoken-chat-state-")) },
  ...
  try {
    const response = await fetch(`${runtime.baseUrl}/v1/chat/completions`, {
      ...
      body: JSON.stringify({ model: "fixture/chat-model", messages: [{ role: "user", content: "rename a to b" }], tools: [{ type: "function", function: { name: "rename_fixture" } }], savetokenTask: "must not leak" }),
    });
...
test("Chat Completions E2E accepts a complete assistant tool-call and tool-result continuation", async () => {
  ...
  const runtime = await startRuntime({ env: { CODEX_HOME: await mkdtemp(join(tmpdir(), "savetoken-chat-codex-")), SAVETOKEN_HOME: await mkdtemp(join(tmpdir(), "savetoken-chat-state-")) }, providers: { fixture: ["chat-model"] }, providerAdapters: [adapter], providerTier: "execution", taskSignals: executionSignals });
  try {
...
test("Chat Completions closes one two-request tool session through the same runtime", async () => {
  ...
  const runtime = await startRuntime({ env: { CODEX_HOME: await mkdtemp(join(tmpdir(), "savetoken-chat-codex-")), SAVETOKEN_HOME: await mkdtemp(join(tmpdir(), "savetoken-chat-state-")) }, providers: { fixture: ["chat-model"] }, providerAdapters: [adapter], providerTier: "execution", taskSignals: executionSignals });
  try {
    const first = await fetch(`${runtime.baseUrl}/v1/chat/completions`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ model: "fixture/chat-model", messages: [{ role: "user", content: "rename a to b" }], tools: [{ type: "function", function: { name: "rename_fixture" } }], savetokenTask: "must-not-leak" }) });
