...
test("service plans are platform-specific but never claim installation was executed", () => {
  expect(createServicePlan("windows", "install", "C:/savetoken-runtime")).toMatchObject({
    status: "NOT_TESTED",
    platform: "windows",
    action: "install",
    executed: false,
  });
  const linuxPlan = createServicePlan("linux", "uninstall", "/opt/savetoken-runtime");
...
test("service and shim plans cover reversible lifecycle actions without executing them", () => {
  for (const platform of ["windows", "macos", "linux"] as const) {
    for (const action of ["install", "start", "stop", "restart", "upgrade", "rollback", "uninstall"] as const) {
      const plan = createServicePlan(platform, action, "C:/isolated/savetoken");
...
test("CLI derives the runtime port from environment and prefers an explicit base URL", () => {
  expect(resolveCliBaseUrl({ SAVETOKEN_PORT: "8789" })).toBe("http://127.0.0.1:8789");
  expect(resolveCliBaseUrl({ SAVETOKEN_PORT: "bad" })).toBe("http://127.0.0.1:8787");
  expect(resolveCliBaseUrl({ SAVETOKEN_BASE_URL: "http://127.0.0.1:9999", SAVETOKEN_PORT: "8789" })).toBe("http://127.0.0.1:9999");
});
