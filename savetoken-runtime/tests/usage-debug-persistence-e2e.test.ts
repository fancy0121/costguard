...
test("isolated runtime persists redacted usage and debug metadata across restart without retaining request body", async () => {
  const root = await mkdtemp(join(tmpdir(), "savetoken-telemetry-"));
  ...
  let first = await startRuntime(options(codexHome, saveTokenHome));
  try {
  ...
  const usagePath = join(saveTokenHome, "telemetry", "usage.json");
  const debugPath = join(saveTokenHome, "telemetry", "debug.json");
  ...
  const second = await startRuntime(options(codexHome, saveTokenHome));
  try {
...
test("runtime refuses an unowned telemetry target instead of overwriting it", async () => {
  const root = await mkdtemp(join(tmpdir(), "savetoken-telemetry-unowned-"));
  const saveTokenHome = join(root, "state");
  const usagePath = join(saveTokenHome, "telemetry", "usage.json");
  await (await import("node:fs/promises")).mkdir(join(saveTokenHome, "telemetry"), { recursive: true });
  await writeFile(usagePath, '{"user":"telemetry"}\n', "utf8");
  try {
    await expect(startRuntime(options(join(root, "codex"), saveTokenHome))).rejects.toThrow("telemetry-state-unverified");
    expect(await readFile(usagePath, "utf8")).toBe('{"user":"telemetry"}\n');
    await expect(access(join(saveTokenHome, "runtime.json"))).rejects.toThrow();
