...
describe("OpenCodex proxy adapter", () => {
  ...
  test("rejects body with savetoken fields (pureBody enforcement)", async () => {
    const adapter = createOpenCodexProxyAdapter({ baseUrl: "http://127.0.0.1:9999" });
    const result = await adapter.invoke({
      requestedModel: "deepseek/deepseek-v4-flash",
      protocol: "responses",
      signal: new AbortController().signal,
      body: { model: "deepseek/deepseek-v4-flash", input: "hello", savetokenTask: "test" },
    });
    expect(result.status).toBe("UNKNOWN");
    expect(result.reason).toBe("proxy-purebody-required");
  });

  test("rejects future SaveToken-prefixed fields before proxy forwarding", async () => {
    const adapter = createOpenCodexProxyAdapter({ baseUrl: "http://127.0.0.1:9999" });
    const result = await adapter.invoke({
      requestedModel: "deepseek/deepseek-v4-flash",
      protocol: "responses",
      signal: new AbortController().signal,
      body: { model: "deepseek/deepseek-v4-flash", input: "hello", savetokenFutureFlag: true },
    });
    expect(result.status).toBe("UNKNOWN");
    expect(result.reason).toBe("proxy-purebody-required");
  });
