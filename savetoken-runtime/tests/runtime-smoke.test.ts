...
test("runtime starts in isolated homes and separates health from readiness", async () => {
  const codexHome = await mkdtemp(join(tmpdir(), "savetoken-codex-") );
  const saveTokenHome = await mkdtemp(join(tmpdir(), "savetoken-state-") );
  ...
  try {
    ...
    expect((await readdir(saveTokenHome)).length).toBeGreaterThan(0);
    expect(JSON.parse(await readFile(join(saveTokenHome, "runtime.json"), "utf8")).baseUrl).toBe(runtime.baseUrl);
  } finally {
    runtime.stop();
  }
});
...
test("runtime does not report ready without a configured provider", async () => {
  const runtime = await startRuntime({
    env: { CODEX_HOME: await mkdtemp(join(tmpdir(), "savetoken-codex-")), SAVETOKEN_HOME: await mkdtemp(join(tmpdir(), "savetoken-state-")) },
    providers: {},
  });
...
test("runtime derives route admission tier from the registered explicit model descriptor", async () => {
  ...
  try {
    ...
    expect(JSON.parse(sol.headers.get("x-savetoken-route-admission")!)).toMatchObject({ requestedTier: "sol", selectedProviderTier: "sol", decidingTier: "sol" });
