...
test("Anthropic Messages E2E preserves native content, tool_use terminal state, and usage", async () => {
  ...
  const runtime = await startRuntime({
    env: { CODEX_HOME: await mkdtemp(join(tmpdir(), "savetoken-msg-codex-")), SAVETOKEN_HOME: await mkdtemp(join(tmpdir(), "savetoken-msg-state-")) },
  ...
  try {
    const response = await fetch(`${runtime.baseUrl}/v1/messages`, {
      ...
      body: JSON.stringify({ model: "fixture/claude-like", max_tokens: 32, messages: [{ role: "user", content: "classify A" }], tools: [{ name: "classify_fixture", input_schema: { type: "object" } }], savetokenRoute: "must-not-leak" }),
    });
...
test("Anthropic Messages E2E accepts a complete tool_use and tool_result continuation", async () => {
  ...
  const runtime = await startRuntime({ env: { CODEX_HOME: await mkdtemp(join(tmpdir(), "savetoken-msg-codex-")), SAVETOKEN_HOME: await mkdtemp(join(tmpdir(), "savetoken-msg-state-")) }, providers: { fixture: ["claude-like"] }, providerAdapters: [adapter], providerTier: "execution", taskSignals: executionSignals });
  try {
...
test("Anthropic Messages closes one two-request tool session through the same runtime", async () => {
  ...
  const adapter: ProviderAdapter = {
    ...
    invoke: async (request) => {
      ...
      expect(Object.keys(body).some((key) => key.toLowerCase().startsWith("savetoken"))).toBe(false);
      if (calls === 1) {
  ...
  const runtime = await startRuntime({ env: { CODEX_HOME: await mkdtemp(join(tmpdir(), "savetoken-msg-codex-")), SAVETOKEN_HOME: await mkdtemp(join(tmpdir(), "savetoken-msg-state-")) }, providers: { fixture: ["claude-like"] }, providerAdapters: [adapter], providerTier: "execution", taskSignals: executionSignals });
  try {
    const first = await fetch(`${runtime.baseUrl}/v1/messages`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ model: "fixture/claude-like", max_tokens: 32, messages: [{ role: "user", content: "classify A" }], tools: [{ name: "classify_fixture", input_schema: { type: "object" } }], savetokenTask: "must-not-leak" }) });
