...
export async function restoreOwnedStateWithCodexProjection(saveTokenHome: string, codexHome: string): Promise<CatalogOperationResult> {
  const projection = join(codexHome, "opencodex-catalog.json");
  try {
    await readFile(projection, "utf8");
  } catch {
    const catalog = await restoreOwnedState(saveTokenHome);
...
export async function uninstallOwnedState(home: string): Promise<UninstallResult> {
  ...
  async function walk(dir: string): Promise<void> {
    for (const entry of await readdir(dir, { withFileTypes: true })) {
      ...
      if ((await readFile(path, "utf8")) !== "savetoken\n") continue;
...
export async function uninstallOwnedStateWithCodexProjection(saveTokenHome: string, codexHome: string): Promise<UninstallResult> {
  const local = await uninstallOwnedState(saveTokenHome);
