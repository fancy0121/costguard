...
export type ProxyModelEntry = { providerId: string; modelId: string; tier: SaveTokenTier; capabilities?: string[] };
...
function makeAdapter(options: OpenCodexProxyOptions, entry: ProxyModelEntry): ProviderAdapter {
  ...
  async function streamInvoke(req: ProviderInvocationRequest): Promise<Response | null> {
    ...
    if (Object.keys(rec).some((key) => key.toLowerCase().startsWith("savetoken"))) return null;
  ...
  async function invoke(req: ProviderInvocationRequest): Promise<ProviderInvocationResult> {
    ...
    const rec = raw as Record<string, unknown>;
    if (Object.keys(rec).some((key) => key.toLowerCase().startsWith("savetoken"))) return { status: "UNKNOWN", actualRuntimeModel: "UNKNOWN", reason: "proxy-purebody-required" };
    const b = bodyFor(entry.providerId, req.protocol, rec);
    try {
