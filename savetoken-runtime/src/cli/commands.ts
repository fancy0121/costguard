export type CliCommand = "status" | "ready" | "models" | "providers" | "usage" | "logs" | "restore" | "uninstall" | "install" | "sync" | "doctor" | "start" | "stop";
...
export function parseCliArgs(args: string[]): ParsedCli {
  ...
  const known = new Set(["status", "ready", "models", "providers", "usage", "logs", "restore", "uninstall", "install", "sync", "doctor", "start", "stop"]);
...
export async function runCli(args: string[], request: CliRequest): Promise<unknown> {
  ...
  if (parsed.command === "start" || parsed.command === "stop") return { status: "UNKNOWN", failClosed: true, reason: "cli-process-command-requires-local-entrypoint" };
