...
export async function startRuntimeProcess(options: { home: string; port: number; providers: Record<string, string[]>; readyTimeoutMs?: number }): Promise<RuntimeProcessResult> {
  ...
  try {
    const child = Bun.spawn({
      ...
      env: { ...process.env, SAVETOKEN_HOME: options.home, CODEX_HOME: join(options.home, "codex-home"), SAVETOKEN_PORT: String(options.port), SAVETOKEN_PROVIDERS_JSON: JSON.stringify(options.providers) },
