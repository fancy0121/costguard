...
// T9: full SaveToken runtime chain (execution tier routing) for DeepSeek
{
  const root = await mkdtemp(join(tmpdir(), "savetoken-deepseek-deep-"));
  try {
    ...
    try {
      const resp = await fetch(`${runtime.baseUrl}/v1/responses`, {
        method: "POST", headers: { "content-type": "application/json" },
        body: JSON.stringify({ model: MODEL, input: "Return exactly the token SAVETOKEN_CHAIN_OK." }),
      });
      const j = await resp.json().catch(() => ({})) as Record<string, unknown>;
      const model = typeof j?.model === "string" ? j.model : "UNKNOWN";
      const text = extractText(j?.output);
      const admissionHeader = resp.headers.get("x-savetoken-route-admission");
      const modelOk = model === "deepseek-v4-flash" || model === "deepseek/deepseek-v4-flash";
      results.push({
        id: "T9", title: "SaveToken runtime full chain", status: resp.status === 200 && modelOk && text.includes("SAVETOKEN_CHAIN_OK") ? "PRESENT" : "FAILED",
    ...
    results.push({ id: "T9", title: "SaveToken runtime full chain", status: "UNKNOWN", httpStatus: "UNKNOWN", facts: { reason: e instanceof Error ? e.name : "runtime-failed" } });
  }
}

const evidence = {
  date: "2026-08-15",
  endpoint: "loopback OpenCodex proxy + SaveToken runtime",
  model: MODEL,
...
const outPath = join(outputDir, "savetoken-deepseek-deep-verification-2026-08-15.json");
