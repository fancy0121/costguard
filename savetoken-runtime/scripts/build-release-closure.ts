...
const stageRoot = join(outputRoot, "savetoken-v0.1.0-mvp");
const manifestPath = join(outputRoot, "savetoken-v0.1.0-mvp-manifest.json");
const archivePath = join(outputRoot, "savetoken-v0.1.0-mvp.zip");
...
const manifest = { package: "savetoken", version: "0.1.0", generatedAt: new Date().toISOString(), fileCount: entries.length, totalBytes: entries.reduce((n, e) => n + e.size, 0), files: entries };
