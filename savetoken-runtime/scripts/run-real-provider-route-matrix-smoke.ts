...
type Route = { name: string; provider: string; model: string; tier: SaveTokenTier; signals: SaveTokenTaskSignals };
...
const root = await mkdtemp(join(tmpdir(), "savetoken-real-route-smoke-"));
const results: Array<Record<string, unknown>> = [];

try {
  for (const route of routes) {
    const runtime = await startRuntime({
      env: { CODEX_HOME: join(root, `${route.name}-codex`), SAVETOKEN_HOME: join(root, `${route.name}-state`) },
    ...
    try {
      const response = await fetch(`${runtime.baseUrl}/v1/responses`, {
        ...
        body: JSON.stringify({ model: `${route.provider}/${route.model}`, input: "Return exactly the token SAVETOKEN_SMOKE_OK.", savetokenTask: route.signals.text }),
      });
  ...
  await writeFile(join(outputDir, "savetoken-real-provider-route-matrix-smoke-2026-08-13.json"), `${JSON.stringify({ date: "2026-08-13", endpoint: "loopback OpenCodex proxy", requestCount: routes.length, results }, null, 2)}\n`, "utf8");
}
