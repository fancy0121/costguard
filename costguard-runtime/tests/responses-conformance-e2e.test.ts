...
async function runtimeWith(adapter: ProviderAdapter) {
  return startRuntime({
    env: { CODEX_HOME: await mkdtemp(join(tmpdir(), "costguard-responses-codex-")), COSTGUARD_HOME: await mkdtemp(join(tmpdir(), "costguard-responses-state-")) },
...
test("Responses E2E validates input before adapter dispatch and returns a stable redacted failure", async () => {
  ...
  try {
    const response = await fetch(`${runtime.baseUrl}/v1/responses`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ model: "fixture/model", input: { invalid: true }, costguardTask: "hidden task" }) });
...
test("Responses E2E preserves a function call, rejects missing continuation output, and never leaks control fields", async () => {
  ...
  try {
    const initial = await fetch(`${runtime.baseUrl}/v1/responses`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ model: "fixture/model", input: "use fixture tool", costguardTask: "must-not-leak", costguardTier: "execution", tools: [{ type: "function", name: "fixture_tool", parameters: { type: "object" } }] }) });
