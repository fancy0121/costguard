...
test("restore removes only CostGuard-owned lines and preserves user edits", () => {
  const owned = "model_provider = \"costguard\"";
...
test("restore never removes an unmarked user assignment", () => {
  const input = [
    "model = \"user-model\"",
    "# costguard-owned: model_provider",
    "",
  ].join("\n");
...
test("restore removes a hashed owned assignment but preserves a later user edit", () => {
  const original = "model_provider = \"costguard\"";
