...
test("authenticated loopback discovery persists only allowlisted route metadata and never changes readiness", async () => {
  const root = await mkdtemp(join(tmpdir(), "costguard-discovery-cache-"));
...
test("unowned discovery cache fails closed before a discovery request can overwrite it", async () => {
  const root = await mkdtemp(join(tmpdir(), "costguard-discovery-cache-unowned-"));
  const costGuardHome = join(root, "state");
  const cachePath = join(costGuardHome, "provider-discovery.json");
  ...
  const runtime = await startRuntime({
    env: { CODEX_HOME: join(root, "codex"), COSTGUARD_HOME: costGuardHome }, providers: { deepseek: ["deepseek-v4-flash"] }, managementToken: token,
    proxyDiscoveryBaseUrl: proxy.url.toString().replace(/\/$/, ""),
  });
  try {
