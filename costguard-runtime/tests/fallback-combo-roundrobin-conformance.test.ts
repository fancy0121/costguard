...
test("runtime executes a configured execution combo with route admission", async () => {
  ...
  const runtime = await startRuntime({
    env: { CODEX_HOME: await mkdtemp(join(tmpdir(), "combo-codex-")), COSTGUARD_HOME: await mkdtemp(join(tmpdir(), "combo-state-")) },
  ...
  try {
    ...
    expect(JSON.parse(first.headers.get("x-costguard-route-admission")!)).toMatchObject({ logicalComboId: "execution-fast", selectedProviderTier: "execution" });
...
test("runtime combo excludes an unknown peer rather than invoking or promoting it", async () => {
  const calls: string[] = [];
  const runtime = await startRuntime({
    env: { CODEX_HOME: await mkdtemp(join(tmpdir(), "combo-unknown-codex-")), COSTGUARD_HOME: await mkdtemp(join(tmpdir(), "combo-unknown-state-")) },
    providers: { deepseek: ["deepseek-v4-flash"], openai: ["gpt-5.6-luna"] },
