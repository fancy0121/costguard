...
async function runtimeFor(
  ...
  return startRuntime({
    env: {
      CODEX_HOME: await mkdtemp(join(tmpdir(), "costguard-codex-")),
      COSTGUARD_HOME: await mkdtemp(join(tmpdir(), "costguard-state-")),
    },
...
test("dispatch strips CostGuard control fields before invoking the provider", async () => {
  ...
  try {
    const result = await fetch(`${runtime.baseUrl}/v1/responses`, {
      ...
      body: JSON.stringify({
        ...
        costguardTask: "classify this request",
        costguardTier: "execution",
        costguardRoute: "fixture/model-a",
      }),
    });
...
test("dispatch strips every top-level costguard-prefixed field before invoking the provider", async () => {
  const observedBodies: unknown[] = [];
  const runtime = await runtimeFor("responses", [], [], observedBodies);
  try {
    const result = await fetch(`${runtime.baseUrl}/v1/responses`, {
      method: "POST", headers: { "content-type": "application/json" },
      body: JSON.stringify({ model: "fixture/model-a", input: "hello", costguardTask: "hidden", costguardPrivateDiagnostic: "internal-only", costguardFutureFlag: true }),
    });
    expect(result.status).toBe(200);
...
test("Responses contract returns a protocol-native response and preserves tools", async () => {
  ...
  try {
    ...
    expect(result.status).toBe(200);
    const routeAdmission = result.headers.get("x-costguard-route-admission");
