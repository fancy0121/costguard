...
test("real quality benchmark requires an explicit one-shot authorization and fails closed by default", () => {
  ...
  expect(benchmarkAuthorization({ COSTGUARD_ALLOW_REAL_BENCHMARK: "1" })).toEqual({ status: "PRESENT" });
  expect(benchmarkAuthorization({ COSTGUARD_ALLOW_REAL_BENCHMARK: "true" })).toEqual({ status: "UNKNOWN", failClosed: true, reason: "real-provider-benchmark-not-authorized" });
});
