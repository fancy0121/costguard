...
test("isolated config injects a root marker, preserves user content, and restores only an unchanged managed block", async () => {
  const root = await mkdtemp(join(tmpdir(), "costguard-config-contract-"));
  const codexHome = join(root, "codex");
  const costGuardHome = join(root, "costguard");
  const config = join(codexHome, "config.toml");
  await (await import("node:fs/promises")).mkdir(codexHome, { recursive: true });
  await writeFile(config, 'model = "user-choice"\n[features]\nflag = true\n', "utf8");
  const runtime = await startRuntime(options(codexHome, costGuardHome));
  try {
    ...
    expect(injected).toContain("# >>> costguard managed proxy >>>");
...
test("isolated config rejects a user-owned proxy root key and a malformed managed block", async () => {
  const root = await mkdtemp(join(tmpdir(), "costguard-config-conflict-"));
  const codexHome = join(root, "codex");
  const costGuardHome = join(root, "costguard");
  const config = join(codexHome, "config.toml");
  await (await import("node:fs/promises")).mkdir(codexHome, { recursive: true });
  ...
  try {
    ...
    await expect(access(join(costGuardHome, "installed.json"))).rejects.toThrow();

    await writeFile(config, "# >>> costguard managed proxy >>>\nopenai_base_url = \"http://127.0.0.1:1/v1\"\n", "utf8");
...
test("isolated config refuses unsupported multiline TOML instead of guessing a root insertion boundary", async () => {
  const root = await mkdtemp(join(tmpdir(), "costguard-config-multiline-"));
  const codexHome = join(root, "codex");
  const costGuardHome = join(root, "costguard");
  const config = join(codexHome, "config.toml");
...
test("isolated managed-config journal only recovers an exact post-image and preserves divergent user bytes", async () => {
  const root = await mkdtemp(join(tmpdir(), "costguard-config-journal-"));
  const codexHome = join(root, "codex");
  const costGuardHome = join(root, "costguard");
  const config = join(codexHome, "config.toml");
  const state = join(costGuardHome, ".costguard-codex-config.json");
  const journal = join(costGuardHome, ".costguard-codex-config.journal");
  await (await import("node:fs/promises")).mkdir(codexHome, { recursive: true });
  await writeFile(config, 'model = "user-choice"\n', "utf8");
  const runtime = await startRuntime(options(codexHome, costGuardHome));
  try {
    ...
    await unlink(state);
    expect(await recoverManagedCodexConfigJournal(codexHome, costGuardHome)).toEqual({ status: "PRESENT" });
    expect(await readFile(state, "utf8")).toBe(stateBody);

    await atomicWriteOwnedJson(journal, JSON.parse(stateBody));
    await writeFile(config, `${await readFile(config, "utf8")}user_added = true\n`, "utf8");
    expect(await recoverManagedCodexConfigJournal(codexHome, costGuardHome)).toEqual({ status: "UNKNOWN", reason: "managed-config-unverified" });
...
test("isolated config preserves CRLF and state stores only hashes, not user config bytes", async () => {
  const root = await mkdtemp(join(tmpdir(), "costguard-config-crlf-"));
  const codexHome = join(root, "codex");
  const costGuardHome = join(root, "costguard");
  const config = join(codexHome, "config.toml");
  const state = join(costGuardHome, ".costguard-codex-config.json");
...
test("concurrent isolated lifecycle installs serialize to one managed root block", async () => {
  const root = await mkdtemp(join(tmpdir(), "costguard-config-concurrent-"));
  const codexHome = join(root, "codex");
  const costGuardHome = join(root, "costguard");
  const config = join(codexHome, "config.toml");
  await (await import("node:fs/promises")).mkdir(codexHome, { recursive: true });
  ...
  try {
    ...
    expect((value.match(/# >>> costguard managed proxy >>>/g) ?? []).length).toBe(1);
    expect((value.match(/# <<< costguard managed proxy <<</g) ?? []).length).toBe(1);
  } finally {
    runtime.stop();
    await rm(root, { recursive: true, force: true });
