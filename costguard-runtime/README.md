...
1. 安装 Bun 1.3.14。
2. 运行 `costguard install`。
3. 运行 `costguard demo`。

从当前源码目录安装命令：`bun install --frozen-lockfile && bun link`。`demo` 是零凭据 loopback 演示，**不是真实 Provider 调用**。

多模型分级路由层：Sol 决策、便宜模型执行，**绝不把高风险任务静默降级**。

> **状态：最小可用工具候选。** 托管 sidecar、命令行和零凭据演示均以本地门禁为准；不代表 OpenCodex 全量 parity、跨平台 hosted CI 或真实 Provider 可用性。
...
安装后的命令包括：`install`、`start`、`status`、`doctor`、`sync`、`stop`、`restore`、`uninstall`、`demo`。托管 OpenCodex 仅绑定 loopback；端口冲突、stale PID、半安装和未知 ownership 均 fail-closed。

To start a local route-preview server with an explicit, credential-free catalog:

```powershell
$env:COSTGUARD_PROVIDERS_JSON='{"openai":["gpt-5.6-sol"]}'
$env:COSTGUARD_PORT='8787'
npx --yes bun@1.3.14 src/index.ts
```

The server writes only to the effective `COSTGUARD_HOME`. Set `CODEX_HOME` to
...
`CostGuardTaskSignals`; without those trusted structured signals, the data
...
When `COSTGUARD_MANAGEMENT_TOKEN` is set by environment reference, the local
...
- Independent Responses, Chat Completions, and Anthropic request parsers and
  native response shapers with tool preservation; CostGuard control fields are
