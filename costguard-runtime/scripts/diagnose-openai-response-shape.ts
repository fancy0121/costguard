...
const root = await mkdtemp(join(tmpdir(), "costguard-shape-diagnostic-"));
...
try {
  ...
  const describe = (value: unknown): unknown => Array.isArray(value)
    ...
    : typeof value;
  console.log(JSON.stringify({ httpStatus: response.status, model: body.model, outputShape: describe(output), routeAdmission: response.headers.get("x-costguard-route-admission") ? "present" : "missing" }));
} finally {
  runtime.stop();
}
