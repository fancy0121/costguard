...
type Fixture = { id: string; category: string; input: string; goal: string; scope: string; nonGoals: string; expectedTier: CostGuardTier; expectedModel: string; acceptance: { type: "json_answer"; expected: { answer: string } } };
...
function tierSignals(fixture: Fixture): CostGuardTaskSignals {
...
const fixturePath = join(root, "docs", "superpowers", "evidence", "costguard-quality-routing-benchmark-fixtures-2026-08-13.json");
...
if (authorization.status !== "PRESENT") {
  ...
  await writeFile(join(outputDir, "costguard-quality-routing-benchmark-2026-08-13.json"), `${JSON.stringify({ date: "2026-08-13", fixtureHash: hash(fixtures), requestCount: 0, status: "UNKNOWN", failClosed: true, reason: authorization.reason }, null, 2)}\n`, "utf8");
  console.log(JSON.stringify({ requestCount: 0, status: "UNKNOWN", reason: authorization.reason }));
  process.exitCode = 1;
} else {
const temporaryRoot = await mkdtemp(join(tmpdir(), "costguard-quality-benchmark-"));
...
for (const fixture of fixtures.tasks) {
  ...
  const runtime = await startRuntime({
    env: { CODEX_HOME: join(temporaryRoot, `${fixture.id}-codex`), COSTGUARD_HOME: join(temporaryRoot, `${fixture.id}-state`) },
  ...
  try {
    ...
    const response = await fetch(`${runtime.baseUrl}/v1/responses`, {
      ...
      body: JSON.stringify({
        model: fixture.expectedModel, input: request, costguardTask: fixture.input,
    ...
    const routeAdmission = response.headers.get("x-costguard-route-admission");
    ...
    results.push({ id: fixture.id, category: fixture.category, requestedModel: fixture.expectedModel, actualModel: "UNKNOWN", httpStatus: "UNKNOWN", usage: "UNKNOWN", acceptance: "UNKNOWN", reason: "costGuard-loopback-proxy-request-failed" });
  } finally {
    runtime.stop();
  }
}
...
const outputDir = join(root, "docs", "superpowers", "evidence");
await mkdir(outputDir, { recursive: true });
await writeFile(join(outputDir, "costguard-quality-routing-benchmark-2026-08-13.json"), `${JSON.stringify({ date: "2026-08-13", fixtureHash: hash(fixtures), requestCount: fixtures.tasks.length, results }, null, 2)}\n`, "utf8");
