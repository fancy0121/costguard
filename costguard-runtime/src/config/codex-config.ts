...
const markerStart = "# >>> costguard managed proxy >>>";
const markerEnd = "# <<< costguard managed proxy <<<";
const configName = "config.toml";
const stateName = ".costguard-codex-config.json";
const journalName = ".costguard-codex-config.journal";
...
function paths(codexHome: string, costGuardHome: string) {
  return { config: join(codexHome, configName), state: join(costGuardHome, stateName), journal: join(costGuardHome, journalName) };
}
...
export async function recoverManagedCodexConfigJournal(codexHome: string, costGuardHome: string): Promise<ManagedConfigResult> {
  const { config, state, journal } = paths(codexHome, costGuardHome);
...
export async function preflightManagedCodexConfig(codexHome: string, costGuardHome: string, baseUrl: string): Promise<ManagedConfigResult> {
  let parsed: URL;
...
export async function injectManagedCodexConfig(codexHome: string, costGuardHome: string, baseUrl: string): Promise<ManagedConfigResult> {
  let parsed: URL;
  try { parsed = new URL(baseUrl); } catch { return { status: "UNKNOWN", reason: "managed-config-format-invalid" }; }
  if (parsed.protocol !== "http:" || !["127.0.0.1", "localhost", "[::1]", "::1"].includes(parsed.hostname) || parsed.pathname !== "/" || parsed.search || parsed.hash || parsed.username || parsed.password) {
    return { status: "UNKNOWN", reason: "managed-config-format-invalid" };
  ...
  const { config, state, journal } = paths(codexHome, costGuardHome);
  try {
    const recovered = await recoverManagedCodexConfigJournal(codexHome, costGuardHome);
    if (recovered.status === "UNKNOWN") return recovered;
    const preflight = await preflightManagedCodexConfig(codexHome, costGuardHome, baseUrl);
...
export async function restoreManagedCodexConfig(codexHome: string, costGuardHome: string): Promise<ManagedConfigResult> {
  const { config, state } = paths(codexHome, costGuardHome);
