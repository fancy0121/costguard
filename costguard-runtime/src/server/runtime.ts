...
import type { CostGuardTaskSignals, CostGuardTier, RouteAdmissionEvidence } from "../types";
...
export type RuntimeOptions = {
  ...
  taskSignals?: (body: Record<string, unknown>) => CostGuardTaskSignals | undefined;
  now?: () => number;
...
function json(value: unknown, status = 200, routeAdmission?: RouteAdmissionEvidence): Response {
  ...
  if (routeAdmission) headers["x-costguard-route-admission"] = JSON.stringify(routeAdmission);
...
function inferredTaskText(body: Record<string, unknown>): { text: string; explicit: boolean } {
  if (typeof body.costguardTask === "string") return { text: body.costguardTask, explicit: true };
...
export async function startRuntime(options: RuntimeOptions): Promise<RuntimeHandle> {
  ...
  const runtimeState = { codexHome: homes.codexHome, costGuardHome: homes.costGuardHome, openCodexHome: homes.openCodexHome, providerCount: Object.keys(options.providers).length };
  ...
  const telemetryRoot = join(homes.costGuardHome, "telemetry");
  const usagePath = join(telemetryRoot, "usage.json");
  const debugPath = join(telemetryRoot, "debug.json");
  const discoveryCachePath = join(homes.costGuardHome, "provider-discovery.json");
  ...
  await atomicWriteOwnedJson(join(homes.costGuardHome, "runtime.json"), runtimeState);
  ...
  ) as Record<string, CostGuardTier>;
  ...
  const management = createManagementHandler({
    ...
    restore: options.restore ?? (() => serializeLifecycle(async () => {
      const config = await restoreManagedCodexConfig(homes.codexHome, homes.costGuardHome);
      if (config.status === "UNKNOWN") return config;
      const projection = await restoreOwnedStateWithCodexProjection(homes.costGuardHome, homes.codexHome);
    ...
    uninstall: options.uninstall ?? (() => serializeLifecycle(async () => {
      const config = await restoreManagedCodexConfig(homes.codexHome, homes.costGuardHome);
      if (config.status === "UNKNOWN") return config;
      const projection = await uninstallOwnedStateWithCodexProjection(homes.costGuardHome, homes.codexHome);
      if (projection.status === "UNKNOWN") return projection;
    ...
    install: () => serializeLifecycle(async () => {
      const statePath = join(homes.costGuardHome, "installed.json");
      const configBaseUrl = `http://127.0.0.1:${server.port}`;
      const recovered = await recoverManagedCodexConfigJournal(homes.codexHome, homes.costGuardHome);
      if (recovered.status === "UNKNOWN") return recovered;
      const configPreflight = await preflightManagedCodexConfig(homes.codexHome, homes.costGuardHome, configBaseUrl);
      ...
      const config = await injectManagedCodexConfig(homes.codexHome, homes.costGuardHome, configBaseUrl);
    ...
    doctor: async () => {
      ...
      const journalPath = join(homes.costGuardHome, ".costguard-owned-batch.json");
      try { await readFile(journalPath, "utf8"); findings.push("journal-residue"); } catch {}
      // Check runtime state
      const statePath = join(homes.costGuardHome, "runtime.json");
      if (await isOwnedJson(statePath)) findings.push("runtime-state-present");
      else findings.push("runtime-state-missing-or-unowned");
      const processState = await inspectRuntimeProcessState(homes.costGuardHome);
  ...
  function analyzeRoute(body: Record<string, unknown>, taskSignalsFn?: (body: Record<string, unknown>) => CostGuardTaskSignals | undefined) {
    const task = inferredTaskText(body);
    const hash = taskTextHash(task.text);
    let trustedSignals: CostGuardTaskSignals | undefined;
  ...
  function admitRoute(analysis: ReturnType<typeof analyzeRoute>, providerTier: CostGuardTier): { evidence: RouteAdmissionEvidence; decision?: ReturnType<typeof decideRoute> } {
 ...
 return json({ status: "UNKNOWN", failClosed: true, reason: "invalid-json" }, 400); }
        ...
        if (body.stream === true && providerRegistry) {
          ...
          return new Response(guardSseTerminal(streamed.response.body, kind, request.signal, isResponses
            ...
            : undefined), { status: 200, headers: { "content-type": "text/event-stream", "cache-control": "no-store", "x-costguard-route-admission": JSON.stringify(evidence) } });
        }
  ...
  await atomicWriteOwnedJson(join(homes.costGuardHome, "runtime.json"), { ...runtimeState, baseUrl });
