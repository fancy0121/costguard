...
export function createServicePlan(platform: ServicePlatform, action: ServiceAction, runtimeRoot: string): ServicePlan {
  ...
  const commands = platform === "windows"
    ? [`schtasks:${action}:costguard:${runtimeRoot}`, `shim:${action}:costguard:${runtimeRoot}`]
    : platform === "macos"
      ? [`launchd:${action}:costguard:${runtimeRoot}`, `shim:${action}:costguard:${runtimeRoot}`]
      : [`systemd-user:${action}:costguard:${runtimeRoot}`, `shim:${action}:costguard:${runtimeRoot}`];
