...
export function packagePathAllowed(relativePath: string): boolean {
  ...
  if (path.startsWith("node_modules/") || path.startsWith(".git/") || path.startsWith(".costguard/")) return false;
  ...
  return ALLOWED_EXACT.has(path) || path.startsWith("bin/") || path.startsWith("src/") || path.startsWith("scripts/") || path.startsWith("tests/");
}
