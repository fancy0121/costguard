...
最终名称：`CostGuard`（原 `savetoken`）。

> 改名说明：项目价值从「省 token」更正为「省钱 + 安全分级」。内部代码标识符（`savetokenTask`、`SAVETOKEN_HOME`、目录 `savetoken-runtime/`、环境变量）暂时保留原前缀，属已知遗留，不影响对外名称；待后续单独重构。
